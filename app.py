...
from assistants import get_article_summarizer, get_thread_writer  # type: ignore

nest_asyncio.apply()
st.set_page_config(
    page_title="Twitter ThreadMaker",
    page_icon=":bird:",
)
st.title("Twitter Thread Maker powered by Groq")
st.markdown("##### :bird: built using [phidata](https://github.com/phidatahq/phidata)")
...
def main() -> None:
    ...
    Twitter_thread_length = st.sidebar.slider(
    ...
    write_article = st.button("Write Thread")
    if write_article:
        news_results = []
            ...
            with st.status("Summarizing News", expanded=False) as status:
                ...
                with st.container():
                    ...
                    for news_result in news_results:
                        ...
                        if _summary_length > Twitter_thread_length:
                            _summary = truncate_text(_summary, Twitter_thread_length)
                            logger.info(f"Truncated summary for {news_result['title']} to {Twitter_thread_length} words.")
                        news_summary += "#### Summary\n\n"
                        news_summary += _summary
                        news_summary += "\n\n---\n\n"
                        if news_summary:
                            summary_container.markdown(news_summary)
                        if len(news_summary.split()) > Twitter_thread_length:
        ...
        Thread_draft  = ""
        Thread_draft  += f"# Topic: {article_topic}\n\n"
        if news_summary:
            Thread_draft  += "## Summary of News Articles\n\n"
            Thread_draft  += f"This section provides a summary of the news articles about {article_topic}.\n\n"
            Thread_draft  += "<news_summary>\n\n"
            Thread_draft += f"{news_summary}\n\n"
            Thread_draft += "</news_summary>\n\n"

        with st.status("Writing Draft", expanded=True) as status:
            with st.container():
                draft_container = st.empty()
                draft_container.markdown(Thread_draft )
            status.update(label="Draft Complete", state="complete", expanded=False)

        article_writer = get_thread_writer(model=writer_model)
        with st.spinner("Writing Thread..."):
            final_report = ""
            final_report_container = st.empty()
            for delta in article_writer.run(Thread_draft):
